...
function view_home_all($data_content, $compare_id,$dk, $html_pages = '', $sort = 0, $viewtype = '')
{
    ...
    if (!empty($data_content)) {
        ...
        if (!$popup) {
            ...
            if (!empty($data_others)) {
                $hmtl = view_home_all($data_others, $compare_id,'main_product_detail.tpl');//view_home_all($data_others, $compare_id);

                $xtpl->assign('OTHER', $hmtl);
                $xtpl->parse('main.product_detail.other');
            }
            if (!empty($array_other_view)) {
                $hmtl = view_home_all($data_others, $compare_id,'main_product_detail.tpl');//view_home_all($array_other_view, $compare_id);
    ...
    if ($pro_config['active_order'] == '1') {
        ...
        if ($data_content['showprice'] == '1') {
           ...
           // if ($data_content['product_number'] > 0 or $pro_config['active_order_number']) {
            ...
            // $group_requie = $pro_config['active_order_popup'] ? 1 : $group_requie;
            // $xtpl->assign('GROUP_REQUIE', $group_requie);
            // $data_link = $link2 . $global_array_shops_cat[$data_content['listcatid']]['alias'].'/'.$data_content['vi_alias'].$global_config['rewrite_exturl'];
            // $xtpl->assign('link_pro', $data_link); 
